...
sequenceDiagram
    ...
    participant McpServer as MCP 計算器伺服器<br/>(港口 8080)
    ...
    %% 處理流程 - 安全內容
    alt 內容安全
        SafetyService->>LangChain: 將提示傳遞給 Bot.chat()
        ...
        LangChain-->>SafetyService: Bot 回應
        SafetyService-->>WebApp: 返回結果映射<br/>{isSafe: "true", botResponse: result, safetyResult: details}
        WebApp-->>User: 顯示計算結果和安全資訊
    else 內容不安全
        SafetyService-->>WebApp: 返回結果映射<br/>{isSafe: "false", safetyResult: details}
